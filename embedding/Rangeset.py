...
class Rangeset():
    ...
    def _update_filters(self, var, val_range, bins):
        ...
        '''
        if len(val_range) == 20:
            self.labels = {key: val for key, val in zip(Category20[20][::-1], val_range[::-1])}
            self.colormap = Category20[20]
            return val_range, val_range
    ...
    def compute_contours(self, var, val_range=None, bins=5):
        ...
        if len(val_range) == 20:
            for v,c in zip(bounds_full, self.colormap):
                df_select = self.df[self.df[var] == v]
